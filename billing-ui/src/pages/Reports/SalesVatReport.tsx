...
import { getSalesGstReport } from "../../api/reportApi";
...
export default function SalesVatReport() {
    usePageTitle("Sales GST Report");
    ...
    const loadReport = async () => {
        try {
            const res = await getSalesGstReport(type, from, to);
            setSummary(res.data.summary);
            setRows(res.data.rows);
            setExpandedRow(null);
        } catch {
            toast.error("Failed to load sales GST report");
        }
    };

    const downloadReportCsv = () => {
        downloadCsv("sales-gst-report", ["Invoice #", "Date", "Customer", "Taxable", "CGST", "SGST", "Total GST", "Total"], rows.map(r => [
            r.referenceNo,
            r.date,
            r.partyName,
            r.taxableAmount,
    ...
    const downloadReportPdf = () => {
        downloadPdf(
            "sales-gst-report",
            "Sales GST Report",
            ["Invoice #", "Date", "Customer", "Taxable", "CGST", "SGST", "Total GST", "Total"],
            rows.map(r => [
                r.referenceNo,
                r.date,
                r.partyName,
